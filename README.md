...
# Installation
For now I suggest in-directory installation (especially for JLab vdi users). Simply copy this project's files into your directory and run:
`make build_it`
Which will install the executable in a `bin/` folder in the current directory.

# Usage
`genAnalysis <input_ROOT_file> <branches.txt>`

This will create a histograms.root file. Note that "selectedBranches.txt" must contain only one branch name per line.
